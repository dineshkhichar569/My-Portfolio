...
function downloadFile(url, filename) {
  // Works when server sends Content-Disposition OR same-origin file.
  // For protected endpoints, user must be authenticated.
...
export default function Documents() {
  ...
  const filteredDocs = useMemo(() => {
    ...
    return base.filter((d) => {
      const hay = `${d.title} ${d.file} ${d.description || ""}`.toLowerCase();
      return hay.includes(q);
    });
  }, [docs, filter, search]);

  return (
    <div className="relative min-h-screen px-4 py-10 text-white bg-gradient-to-b from-black via-black to-zinc-950">
      {/* Back Button */}
      <motion.div
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="space-y-2">
              ...
              <p className="max-w-2xl text-sm text-white/70">
                Click any file to preview & download (served from your secure
                backend).
              </p>
          ...
          {active && (
            <>
              ...
              >
                ...
                <div
                  className="w-full max-w-5xl overflow-hidden rounded-3xl border border-white/10 bg-white/10 shadow-[0_20px_80px_rgba(0,0,0,0.6)] backdrop-blur-xl"
                ...
                >
                  ...
                  <div className="h-[74vh] bg-black/10 p-0">
                    {active.type === "image" ? (
                      ...
                      <div className="h-full w-full overflow-hidden border-t border-white/10 bg-black/40">
                        <iframe
                          title={active.title}
                          src={active.url}
                  ...
                  <div className="flex items-center justify-between border-t border-white/10 px-5 py-3 text-xs text-white/60">
                    <span className="hidden sm:inline">
                      Protected route (requires your backend auth)
                    </span>
